/**
 * Program/Project: Lab-12b: Shopping List - Part 4
 * Your Name: Olivia Jaime and Ronald Britos
 * Date: 03/24/2022
 * Instructor: Jeff Light
 * Description: Opening a Shopping List from Persistent Storage
 */
...
public class Lab12b {
	...
	public static void saveList(Scanner sIn, ArrayList<String> shoppingList, String FILENAME) {
		...
		do {
			System.out.printf("%nAre you sure you wish to overwrite the '%s' file with your current list? (Y/N) ", FILENAME);
			answer = sIn.nextLine();
		...
		if (answer.equalsIgnoreCase("Y")) {
			...
			System.out.printf("Shopping List saved to file: %s.%n", FILENAME);
		}else {
	...
	public static ArrayList<String> openList(Scanner sIn, ArrayList<String> shoppingList, String FILENAME){
		// Declare variables
		//ArrayList<String> savedShoppingList = new ArrayList<String>();
		File outFile = new File (FILENAME);
		...
		// Input validation
		do {
			System.out.print("\nAre you sure you wish to overwrite your current 'Shopping List.txt' file? (Y/N) ");
			String answer = sIn.nextLine();
			// 
			if(answer.equalsIgnoreCase("Y")) {
				// Try to open the file, output a message if not possible.
				try {
					myReader = new Scanner (outFile);
				}catch(Exception e) {
					System.out.printf("File Error: File '%s' not found! Please save the list before trying to open it.", FILENAME);
				}
				
				// Open shoppingList and overwrite it
				while (myReader.hasNext()) {
					shoppingList.add(myReader.nextLine());
				}
				myReader.close();
				
				// Tell the user that the list has been overwritten
				System.out.printf("Shopping List overwritten from file: %s.%n", FILENAME);
