 ...
 *Your Name: Olivia Jaime, Pao Aguiniga
 *Date: 11/9/2019
	...
	public static int addItems(Scanner sIn, ArrayList<String> shoppingList) {
		...
		do {
			...
			if (answer.equals("")) {
				...
				System.out.printf("'%s' has been added to the Shopping List.%n", answer);
				count++;
			}	
		}while(!stop);		
		...
		do {
			...
			if (answer.equals("")) {
				...
				if (shoppingList.contains(answer)) {
					...
					System.out.printf("'%s' has been deleted from the Shopping List.%n", answer);
					count++;
				}else {
